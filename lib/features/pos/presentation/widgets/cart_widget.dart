...
class CartWidget extends StatelessWidget {
  ...
  const CartWidget({super.key, this.showActions = true});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PosBloc, PosState>(builder: (context, state) {
      ...
      return Column(children: [
        ...
        if (showActions) _actions(context, state),
      ]);
    });
  }
        ...
        child: Row(children: [
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            IconButton(
                ...
                onPressed: () => sl<PosBloc>().add(UpdateCartItemQtyEvent(
            ...
            Text(MoneyFormatter.instance.format(item.lineTotal),
            ...
            IconButton(
                ...
                onPressed: () => sl<PosBloc>().add(RemoveFromCartEvent(
                    item.productId,
                    variationId: item.variationId))),
          ]),
        ]));
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        if (state.discountAmount > 0)
          _row('Discount',
              '-${MoneyFormatter.instance.format(state.discountAmount)}',
              valueColor: Colors.red),
        _row('Tax', MoneyFormatter.instance.format(state.tax)),
        const Divider(),
        _row('Total', MoneyFormatter.instance.format(state.total),
            bold: true, valueColor: Theme.of(context).primaryColor),
      ]),
    );
  }
        ...
        child: Row(children: [
          Expanded(
              child: OutlinedButton.icon(
                  onPressed: () => sl<PosBloc>().add(ClearCartEvent()),
